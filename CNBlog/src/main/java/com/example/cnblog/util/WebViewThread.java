...
public class WebViewThread extends Thread {
	...
	public WebViewThread(String url,Handler handler){
		this.urlstr=url;
	...
	public void run() {
		...
		try {
			...
			NodeList n1=doc.getElementsByTagName("Content");
			NodeList n2=doc.getElementsByTagName("Title");
			
			String s=n1.item(0).getFirstChild().getNodeValue();
			String title=n2.item(0).getFirstChild().getNodeValue();
			News n=new News();
			n.setContent(s);
			n.setTitle(title);
			//AppStatic.news=n;
