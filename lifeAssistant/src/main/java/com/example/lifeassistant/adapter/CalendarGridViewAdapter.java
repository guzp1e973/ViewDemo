...
public class CalendarGridViewAdapter extends BaseAdapter {
	...
	public View getView(int position, View convertView, ViewGroup parent) {
		...
		MyCalenderHolder mcHolder = new MyCalenderHolder();
		...
		mcHolder.txtToDay = new TextView(activity);
		mcHolder.txtToDay.setGravity(Gravity.CENTER_HORIZONTAL);
		mcHolder.txtToDay.setTextSize(9);
		...
		if (equalsDate(calToday.getTime(), myDate)) {
			...
			mcHolder.txtToDay.setText(calendarUtil.toString());
		} else {
			mcHolder.txtToDay.setText(calendarUtil.toString());
		}
		...
		if (iMonth == iMonthViewCurrentMonth) {
			mcHolder.txtToDay.setTextColor(resources
					.getColor(R.color.ToDayText));
			txtDay.setTextColor(resources.getColor(R.color.Text));
		} else {
			txtDay.setTextColor(resources.getColor(R.color.noMonth));
			mcHolder.txtToDay.setTextColor(resources.getColor(R.color.noMonth));
		}
		...
		LinearLayout.LayoutParams lp1 = new LinearLayout.LayoutParams(
				LayoutParams.MATCH_PARENT, LayoutParams.WRAP_CONTENT);
		iv.addView(mcHolder.txtToDay, lp1);
		return iv;
	}

	class MyCalenderHolder {
		TextView txtToDay;
	}
